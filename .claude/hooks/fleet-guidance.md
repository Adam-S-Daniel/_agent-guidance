...
supported, never `github-actions` (#133). Version updates only (a security
advisory bypasses it); unset `cooldown` is **not** "no wait" (GitHub's implicit
minimum is 3 days); `semver-minor-days` / `semver-patch-days` stay undefined
and fall back to `default-days`. A package you add or bump **by hand** has no
automation watching it: check `npm view <pkg> time --json`, take the newest
release that has cleared 7 days, pin it exact (no caret).
