...
re-derived by eye each time. It writes nothing — those fields are excluded from
the committed snapshot for the reason the next section gives.
...
### The configuration itself is captured beside this file

[`guidance-centralization.routine.md`](guidance-centralization.routine.md) is a
committed capture of the Routine's **configuration** — schedule, session
binding, attached repositories, connectors, pre-approved tools, notification
channels, and the stored prompt in full with its digest. It is generated by
[`scripts/capture-routine.py`](../../scripts/capture-routine.py) from the same
`list_triggers` response the manual check above reads, so nobody transcribes it
by hand.

It does **not** close the gap above, and should not be read as closing it:
nothing regenerates it on a schedule either, so a Routine that silently stops
being fired leaves that file looking exactly as current as it did the week
before. What it buys is narrower and still worth having — a *change* to the
Routine stops being invisible. An edit made on claude.ai is otherwise a
difference against nothing, because no prior copy exists to diff; with a
snapshot committed, `capture-routine.py --check` answers "has the configuration
moved since anyone reviewed it" in one command, and a real move arrives as a
reviewable diff. It also makes the prompt-versus-spec disagreement §2.5 records
mechanically checkable rather than a claim to re-establish by reading.

Two properties of that file are deliberate and easy to undo by accident.
**Runtime state is excluded** — `next_run_at`, `last_fired_at`, the whole
`last_run` block — because those change on every fire, so capturing them would
make the file stale the moment the Routine runs and turn `--check` permanently
red. **Identifiers are redacted** — the account, the environment, the connector,
session and message uuids — because this repo is public; the *field* is still
listed so its absence reads as classified rather than overlooked. Both are
enforced by the script, and a field it has never classified makes it refuse
outright rather than write a snapshot that still reads as complete.

**CI cannot verify it.** The Routines API has no public REST endpoint to curl
(measured 2026-08-29 — `api.anthropic.com` answers a structured 404 for every
routines-shaped path tried), and a GitHub runner holds no MCP server at all. So
`--check` runs only where `list_triggers` answers, which today means an
interactive session that holds `claude-code-remote`. Whether a **fired** run
holds one is not something to assume either way — it is probe 2 of §0.5 Step 0,
and no fired run has yet reported the answer. A run that finds it answering may
refresh the snapshot and open a pull request for the diff; a run that does not
should say so rather than leave the file's currency implied.
...
### Measured 2026-09-14, fifth fired run

Tool availability repeated the 2026-09-01/09-02/09-06/09-13 shape exactly,
for a fifth consecutive fire: `mcp__Claude_Code_Remote__list_repos`,
`list_triggers` and `add_repo` were all three absent (ToolSearch for the
`Claude_Code_Remote` family returned no matching tool) and the `gh` CLI was
absent too (`which gh` exited 1). Set (a) again `NOT COMPUTABLE (no
enumeration tool available)`, recorded as the one-line repeat this
section's own convention calls for.

Both GitHub MCP connectors answered (`mcp__github__get_me` and
`mcp__github-mcp__get_me` both resolved to the same account), and
`mcp__github__`'s Actions tools (`actions_list`, `actions_get`,
`get_check_run`, `get_job_logs`) were present and schema-loadable — the
fuller profile, matching 2026-09-02/09-06/09-13 rather than 2026-09-01's
narrower one.
...
(`GIT_TERMINAL_PROMPT=0`) for all 22 `repos.yml` names under the owner
recorded in the existing Step 2 mapping: all 22 resolved on the first
probe, no fallback owner needed. 0 unreadable. As in the prior four runs,
`gh repo list ... --json` was unavailable, so this is single-source
verification only, not the dual-source cross-check Step 2 calls for.

`repos.yml` on `main` was unchanged from the 2026-09-13 measurement (still
...
its actual default branch `claude/setup-github-pages-ZCksi`) before being
read, rather than trusted as already current. Every repo's managed half
matched a fresh `./scripts/build-agents-md.sh` run (modulo the documented
command-substitution trailing-newline trim), every repo carried exactly
one `## Repo-specific additions` marker, and every `CLAUDE.md` carried
exactly one line-start `@AGENTS.md` bridge. The drift report
(`drift-report-latest`, generated 2026-09-13 11:14 UTC) agreed on all 19
rows (`up-to-date`, marker `yes`, `bridge-ok`) but was not relied on as the
source of truth per §1 — the direct checks above are what this line cites.

Both previously-flagged centralization candidates remain correctly
resolved (re-confirmed by direct fetch of `agentskills/AGENTS.md` and
`base.md` on `main` rather than cached memory). A full pass over all 19
in-scope repos' `## Repo-specific additions` content, including a grep for
generalizing language (`every repo`, `fleet-wide`, `standing rule`, `general
rule`, etc.) and for base.md's own signature phrases duplicated locally,
surfaced no new promotion candidates and no new redundant copies this run.
Two open PRs exist in `_agent-guidance` (#124, a large paused one-way-door
review effort touching `.claude/hooks/` and `agents-md/base.md`; #111, an
automated `skills_bootstrap` hook-pin bump) — neither is a `repos.yml`
