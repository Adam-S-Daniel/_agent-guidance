...
lock, and if it fails, the federated half has moved. It does not follow — and
the reason it does not follow survives everything the paired generator changed
about that report.

*Measured against the generator this ADR ships beside*, `agentskills`
`ag58-generator` at `c906bbf`, on a real two-registry scenario (a primary plus
`cms-platform`, both real git repos, the lock generated by the generator
itself):

- **only the primary edited, the source exactly at its pin** — exit 1, and one
  `FAILED:` headline naming the PRIMARY's sha. Zero federated differences, and
  a combined verdict that says `FAILED` all the same.
- **only the source edited** — exit 1, one headline, and it names
  `org/cms-platform` and that source's OWN pin, with a
  `--repin --ref <primary> --repin-source 'org/cms-platform@'` under it.
- **both drifted** — exit 1 and TWO headlines, one per drifted source, the
  primary's first.

So the report attributes drift correctly to a human reader. **What it cannot do
is answer a `grep -q '^FAILED:'`**, which is the whole of the signal a shell
gate has: the first case is a primary-only drift that a combined verdict
reports as `^FAILED:` and exit 1, indistinguishable from the third. A gate
keyed on it reads "the primary moved" as "the federated half moved". Every
consumer lock in the fleet would have every federated pin advanced to that
source's HEAD on any night the primary registry had a bundle-touching commit —
which is the routine case the bumper exists to handle. That is not churn; it is
an unasked-for advance of a pin 0005 reserved for a human, fanned out across
the fleet, in a diff whose visible content is digests.

*A note on this paragraph's history, because it is the kind of evidence that
goes stale silently.* It first recorded three bullets measured against the
generator as it stood BEFORE the paired PR — where `check_current` returned one
flat list of differences across every source and `main` printed a single
headline anchored on the lock's top-level `ref`. Two of those bullets became
checkably false the moment the paired generator grouped by source: a source-only
drift no longer names the primary's clean sha, and three drifted sources no
longer give one headline. The decision was right and its stated evidence was
not, which is worse than vague evidence — it is quotable and wrong. The
measurement above is dated to the generator it was taken against; re-take it
rather than trusting it if that file moves again.
...
- **Three newly reachable red nights**, per the decision above. The first two
  — the cross-registry basename collision and the shrink guard — become
  reachable the first time a pin advances. The third is present from the first
  run: a lock that federates ONE registry twice, which is representable and
  `--check`-green because the generator's uniqueness test is keyed on bundle.
  Its scoped question answers `FAILED`, and the `--repin-source` that follows
  is refused, because one spec names two entries and advancing "it" has two
  answers. Counted, lock left alone, red — and the cost is stated rather than
  softened: that consumer gets no pull request at all, including for a
  legitimately stale primary, until a human gives the two halves two names.
  Dropping the ambiguous source and re-pinning the rest was considered and
  rejected on the same grounds as the other two: half-re-pinning a federated
  lock is ADR 0001's damage, and no retry can make the adjudication. No lock in
  the fleet has that shape today, and five consumer locks remain unread.
- **The suite still exercises a stand-in generator.** The stub in
  ...
  That parity is a standing debt, not a one-time job: adversarial review found
  four refusals the real generator had and the stand-in did not — a registry
  federated twice, a source or primary pinned at a branch name, the
  source-checkout identity probe, and `--only ''` slipping a truthiness guard —
  plus the report-side rule that a block whose repair would be refused carries
  no command at all. `test_bump_stub_generator_parity` asserts them one argv at
  a time, so the list is something that can go red rather than something a
  comment claims.
